...
async def menu_reviews(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    logger.info("Opening reviews from menu for user_id=%s", user_id)
    await state.clear()
    # Редактируем текущее сообщение (меню) в список отзывов, чтобы избежать дублирования
...
async def reviews_callbacks(callback: CallbackQuery, state: FSMContext) -> None:
    ...
    if action in ("list", "page", "refresh", "list_page"):
        logger.info("Reviews: action=%s, category=%s, page=%s, user_id=%s", action, category, page, user_id)
        
        # При возврате к списку с карточки (action="list") редактируем текущее сообщение
        edit_current = (action == "list")
        
        logger.info("Reviews: calling _show_reviews_list, edit_current_message=%s", edit_current)
        await _show_reviews_list(
            user_id=user_id,
            category=category,
            page=page,
        ...
        logger.info("Reviews: _show_reviews_list completed")
        return
