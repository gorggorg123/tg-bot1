...
MAX_REVIEW_LEN = 450
MAX_REVIEWS_LOAD = 1000  # ✅ ОПТИМИЗАЦИЯ: Уменьшено с 2000 для быстрой загрузки
MSK_SHIFT = timedelta(hours=3)
MSK_TZ = timezone(MSK_SHIFT)
TELEGRAM_SOFT_LIMIT = 4000
REVIEWS_PAGE_SIZE = 10
CACHE_TTL_SECONDS = 180  # ✅ ОПТИМИЗАЦИЯ: Увеличено с 120 до 3 минут
...
# ✅ НОВОЕ: Кеш для списка отзывов (3 минуты)
_reviews_list_cache = CacheRepository(
    name="reviews_list",
    memory_ttl=180,  # 3 минуты
...
def format_review_card_text(
    ...
    *,
    ...
    """Собираем карточку отзыва в современном читабельном стиле.
    ...
    # ═══ Заголовок ═══
    lines.append(f"📝 <b>Отзыв</b> {stars} • {badge}")
    lines.append(f"📅 {created_part}{age_part}")
    lines.append(f"🛒 Товар: {product_label}")

    if card.id:
        lines.append(f"<code>ID: {card.id}</code>")

    # ═══ Текст отзыва ═══
    lines.append("")
    lines.append("<b>Текст отзыва:</b>")
...
async def fetch_recent_reviews(
    ...
    *,
    ...
    ✅ ОПТИМИЗАЦИЯ: Кеширует результаты на 3 минуты
    """
    
    # Проверяем кеш
    cache_key = f"reviews:days:{days}:limit:{limit_per_page}"
    cached = await _reviews_list_cache.get(cache_key)
    if cached is not None:
        logger.debug("Reviews list cache HIT")
        cards, pretty = cached
        return cards, pretty
    
    logger.debug("Reviews list cache MISS, fetching from API")
    ...
    try:
        ...
        cache_key = f"reviews:days:{days}:limit:{limit_per_page}"
        await _reviews_list_cache.set(cache_key, (filtered_cards, pretty))
    ...
    *,
    ...
    for i, card in enumerate(slice_items, start=1 + safe_page * page_size):
        ...
        if review_id:
            ...
            # Формат: "01) ✅ 24.01.2026 20:09 | ★4  Название товара..."
            idx = f"{i:02d})"
            
            # Фиксированная длина для названия товара (с учётом общего лимита ~64)
            TARGET_PROD_LEN = 24
            
            # Обрезаем или дополняем название товара до фиксированной длины
            if len(prod) > TARGET_PROD_LEN:
                prod_fixed = prod[:TARGET_PROD_LEN - 1] + "…"
            else:
                # Дополняем NO-BREAK SPACE до фиксированной длины
                prod_fixed = prod + "\u00A0" * (TARGET_PROD_LEN - len(prod))
            
            # Формируем label
            label = f"{idx} {badge} {created} | {rating_txt}  {prod_fixed}"
