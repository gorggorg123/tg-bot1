...
CHAT_AUTOREPLY_STATE_FILE = ROOT / "chat_autoreply_state.json"
...
def mark_chat_activated(user_id: int, chat_id: str) -> bool:
    """Помечает чат как активированный.
    
    Returns:
        bool: True если это новая активация (чат не был активирован ранее), False если уже был активирован
    """
    _ensure_loaded()
    cid = str(chat_id).strip()
    if not cid:
...
def get_chat_autoreply_state() -> dict:
    """Читает состояние автоответов из файла.
    
    Returns:
        dict: {"last_processed": {"<chat_id>": <int last_message_id>}, "updated_at": "..."}
    """
    try:
        if not CHAT_AUTOREPLY_STATE_FILE.exists():
            return {"last_processed": {}, "updated_at": _utc_now_iso()}
        raw = CHAT_AUTOREPLY_STATE_FILE.read_text(encoding="utf-8", errors="replace").strip()
        if not raw:
            return {"last_processed": {}, "updated_at": _utc_now_iso()}
        data = json.loads(raw)
        if not isinstance(data, dict):
...
def save_chat_autoreply_state(state: dict) -> None:
    """Сохраняет состояние автоответов в файл.
    
    Args:
        state: dict с ключами "last_processed" (dict[str, int]) и "updated_at" (str)
    """
    try:
        state["updated_at"] = _utc_now_iso()
        _write_json_atomic(CHAT_AUTOREPLY_STATE_FILE, state)
    except Exception as e:
        logger.warning("Failed to save chat autoreply state: %s", e)
...
__all__ = [
    ...
    "mark_chat_activated",
    "get_user_settings",
    "get_chat_autoreply_state",
    "save_chat_autoreply_state",
    "flush_storage",
]
