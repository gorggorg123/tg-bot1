...
class ChatMediaPreviewTest(unittest.TestCase):
    def test_detect_media_kind(self):
        self.assertEqual(chat_handlers._detect_media_kind(url="https://example.com/pic.JPG"), "photo")
        self.assertEqual(chat_handlers._detect_media_kind(url="https://example.com/movie.mp4"), "video")
        self.assertEqual(chat_handlers._detect_media_kind(url="https://example.com/file.pdf"), "document")
        self.assertEqual(
            chat_handlers._detect_media_kind(
                url="https://example.com/blob", filename="name.bin", content_type="image/jpeg"
            ),
            "photo",
        )

    def test_send_bubbles_sends_document_for_pdf(self):
        async def run_case():
            key = chat_handlers._bkey(10, "cid-pdf")
            chat_handlers._CHAT_BUBBLES.pop(key, None)
            sent_docs: list[tuple[str, str | None]] = []
            ...
            class DummyBot:
                ...
                async def send_message(self, chat_id, text, parse_mode=None, disable_web_page_preview=None):
                    ...
                    msg = Msg()
                    msg.message_id = 100 + self._counter
                    return msg

                async def send_photo(self, *args, **kwargs):
                    raise AssertionError("send_photo should not be used for pdf")

                async def send_video(self, *args, **kwargs):
                    raise AssertionError("send_video should not be used for pdf")

                async def send_document(self, chat_id, document, caption=None, parse_mode=None):
                    sent_docs.append((getattr(document, "filename", ""), caption))
                    self._counter += 1
                    ...
                    msg = Msg()
                    msg.message_id = 200 + self._counter
                    return msg

            with patch.object(
                chat_handlers,
                "download_chat_file",
                return_value=(b"%PDF-1.4", "guide.pdf", "application/pdf"),
            ):
                msgs = [
                    NormalizedMessage(
                        role="seller",
                        text="",
                        media_urls=["https://api-seller.ozon.ru/v2/chat/file/abc123.pdf"],
                    )
                ]
                await chat_handlers._send_bubbles(DummyBot(), 1, 10, "cid-pdf", msgs)

            chat_handlers._CHAT_BUBBLES.pop(key, None)
            return sent_docs, sent_messages

        sent_docs, sent_messages = asyncio.run(run_case())
        self.assertEqual(len(sent_docs), 1)
        self.assertEqual(sent_docs[0][0], "guide.pdf")
        self.assertIn("📎 файл", sent_docs[0][1] or "")
        self.assertFalse(any("не удалось встроить медиа" in msg for msg in sent_messages))
