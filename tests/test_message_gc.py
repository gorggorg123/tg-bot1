...
class MessageGCTriggerPreserveRegressionTest(unittest.TestCase):
    def setUp(self):
        self.tmp_dir = tempfile.TemporaryDirectory()
        section_refs_store._reset_for_tests(Path(self.tmp_dir.name) / "section_refs.json")
    ...
    def test_trigger_is_deleted_when_new_section_message_is_sent(self):
        user_id = 11
        chat_id = 77
        trigger_mid = 500

        section_refs_store.set_ref(user_id, message_gc.SECTION_CHATS_LIST, chat_id, trigger_mid)
        section_refs_store.set_ref(user_id, message_gc.SECTION_CHAT_HISTORY, chat_id, trigger_mid)
        ...
        result = asyncio.run(
            message_gc.render_section(
                message_gc.SECTION_CHAT_PROMPT,
                bot=bot,
                chat_id=chat_id,
                user_id=user_id,
                text="prompt",
        ...
        self.assertIn(trigger_mid, bot.deleted)
        self.assertIsNone(message_gc.get_section_ref(user_id, message_gc.SECTION_CHATS_LIST))
        self.assertIsNone(message_gc.get_section_ref(user_id, message_gc.SECTION_CHAT_HISTORY))
        self.assertEqual(message_gc.get_section_message_id(user_id, message_gc.SECTION_CHAT_PROMPT), 600)
